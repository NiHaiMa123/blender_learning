"""Phase 25: NEW tree built from measured spec (MAPLE_TREE_SPEC.md).
Trunk = switchback column (elbow at 52% Ht), few long smooth limbs,
shallow flutes only, smooth dark bark. Mid render only."""
...
for i, ang in enumerate((-0.7, 0.4, 2.5, 3.7)):
    dx, dy = math.cos(ang) * 1.3, math.sin(ang) * 1.1
    limb(f'root{i}', [
        ((BX - 0.05, 0.0, 0.35), 0.34),
        ((BX - 0.05 + dx * 0.45, dy * 0.45, -0.05), 0.19),
        ((BX - 0.05 + dx * 0.8, dy * 0.8, -0.35), 0.11),
        ((BX - 0.05 + dx, dy, -0.75), 0.06),
...
s.render.filepath = os.path.join(RENDER_DIR, 'p25_mid.png')
bpy.ops.render.render(write_still=True)
print('RENDERED p25_mid')
