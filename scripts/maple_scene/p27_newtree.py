"""Phase 27: match EXTRACTED silhouette — smooth elbow bulge (not step),
waist pinch at ~0.72Ht, swollen fork mass, fatter co-leader."""
...
trunk_spec = [
    (0.00,  0.00, 0.115),   # base — extracted silhouette: dia 0.23 Ht
    (0.15, -0.05, 0.100),
    (0.35, -0.10, 0.088),
    (0.52, -0.145, 0.082),  # left elbow — smooth bulge, not a step
    (0.62, -0.12, 0.072),
    (0.72, -0.07, 0.060),   # WAIST: pinch before fork
    (0.82, -0.02, 0.072),   # fork swells (limbs converge)
    (0.92, -0.06, 0.058),
...
limb('coLeader', [
    ((BX - 0.86, 0.05, 6.90), 0.40),
    ((BX - 1.45, 0.15, 7.75), 0.32),
    ((BX - 2.20, 0.28, 8.50), 0.23),
    ((BX - 3.00, 0.38, 9.15), 0.14),
    ((BX - 3.75, 0.45, 9.70), 0.06),
], soft_star, twist_total=math.radians(30), mat=bark_fine)
...
collars = [
    ...
    (BX - 0.35, 0.05, 5.95, 0.34),   # fork mass swell
    (BX - 0.55, 0.08, 6.55, 0.26),
    (BX - 0.70, 0.05, 6.75, 0.22),
...
s.render.filepath = os.path.join(RENDER_DIR, 'p27_mid.png')
bpy.ops.render.render(write_still=True)
print('RENDERED p27_mid')
